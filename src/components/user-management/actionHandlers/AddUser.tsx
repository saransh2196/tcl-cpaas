...
import { addUserSchema } from '../../../utils/yupschemas'
import { CommonInput } from '../../common/elements/CommonInput'
import DeleteOutlineIcon from '@mui/icons-material/DeleteOutline';
import { userLoginData } from '../../../services/api'
import { addUser, loadInputValues } from '../../../redux/slices/userManagmentSlice';
import {
...
import { AddUserBody } from '../../../utils/ApiRequestbody'
...
const AddUser = ({ handleShowAddUser, showAddUser }: any) => {
  const { t } = useLocales();
  ...
  const submitaddUser = () => {
    ...
    const body = AddUserBody({
      cloneUserEmail: type == '1' ? fv.userValue : '',
      email: fv.username,
    ...
    dispatch(addUser(body, (status: boolean) => {
      if (status) {
        setinviteSent(true);
        handleShowAddUser(false);
      } else {
        setShowError(true);
      }
    }))
    ...
    <>
      <Box
        sx={{
          ...
          transform: `translateX(${showAddUser ? 0 : 100}%)`,
        }}
            ...
            <div className="customScroll">
                  ...
                  <Grid item xs={6}>
                    <Box sx={loaclInputGrid}>
                      <DropDownInput
                        ...
                        label={'adduserSelectRole'}
                        data={[{ name: 'User', value: 'User' }, { name: 'Admin', value: 'Admin' }]}
                        fieldName={'role'}
                        formState={formState}
                        typeName={'dropdown'}
                      />
              ...
              <Box className='basicModule'>
                <FormControl>
                  ...
                  >
                    <FormControlLabel value={'1'} control={<Radio checked={type == '1'} onClick={() => (shiftType('1'))} sx={radioClr} />} label={t<string>('CloneExistingProfile')} />
                    <FormControlLabel value={'2'} control={<Radio checked={type == '2'} onClick={() => (shiftType('2'))} sx={radioClr} />} label={t<string>('CreateNew')} />
                  </RadioGroup>
                </FormControl>
              </Box>
              ...
              <Box className='basicModule' sx={{ mt: '30px', mb: '20px', }}>
                <Stack>
                  <Typography
                    ...
                    className="raise-ticket--typography title-small pmtext1"
                  >
                    {t<string>('CreateNewProfile')}
                  </Typography>
                </Stack>
              </Box>

              {type == '1' && <Box sx={{ display: 'flex' }}>
                <Box sx={{ ...loaclInputGrid, flexBasis: 'calc((100% - 60px)/2)' }}>
                  <DropDownInput
              ...
              <Box className='basicModule' sx={{ mt: '15px' }}>
                <Stack>
              ...
              <Box className='basicModule tableModule' >
                <Stack className="profile-details addUser-details" style={{ width: '100%' }}>
                  <Grid container spacing={2} className="spacinglarg" sx={{ padding: '0px 20px 10px', }}>
                    <Grid item xs={3}>
                      <Typography variant="h6" component="h2" className="labelText1">
      ...
      <ModalInviteSent inviteSent={inviteSent} setinviteSent={setinviteSent} email={email} />
      <ModalTicketError data={{ text: "Unable to add user" }} showError={showError} setShowError={setShowError} />
